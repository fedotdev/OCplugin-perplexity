...
> [!IMPORTANT]
> Ответы Perplexity — результат сторонней системы, а не первоисточник. Для инженерных и научных задач проверяйте каждое утверждение по источнику из `sources`. Каждый успешный результат снабжается преамбулой «НЕДОВЕРЕННЫЕ ДАННЫЕ».
